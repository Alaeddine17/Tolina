...
if (isset($_POST["Envoyer"])) {
    ...
    if (isset($_SESSION["shopping_cart"])) {
        ...
        if (!in_array($_GET["Id"], $item_array_id)) {
            ...
            echo '<script>window.location="glaces.php"</script>';
        } else {
...
unset($_SESSION["shopping_cart"][$keys]);
echo '<script>alert("Commande est supprimer")</script>';
echo '<script>window.location="glaces.php"</script>';
		}
	}
}
...
?>
	...
	<section class="glaces container">
		...
		><span class="text-white font-bold"><?php echo $row['Prix']?> MAD</span></div>
			...
			<form method="POST" action="glaces.php?action=add&Id=<?php echo $row['Id']?>">


				<img class="px-6 pt-6 pb-3 rounded-t-lg" src="./glaces/<?php echo $row['Image']?>" alt="product image" />
    ...
    <path d="M8.574 11.024h6.852a2.075 2.075 0 0 0 1.847-1.086 1.9 1.9 0 0 0-.11-1.986L13.736 2.9a2.122 2.122 0 0 0-3.472 0L6.837 7.952a1.9 1.9 0 0 0-.11 1.986 2.074 2.074 0 0 0 1.847 1.086Zm6.852 1.952H8.574a2.072 2.072 0 0 0-1.847 1.087 1.9 1.9 0 0 0 .11 1.985l3.426 5.05a2.123 2.123 0 0 0 3.472 0l3.427-5.05a1.9 1.9 0 0 0 .11-1.985 2.074 2.074 0 0 0-1.846-1.087Z"/>
		...
		<?php
            ...
            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                ...
                <td class="px-6 py-4">
                    <a href="glaces.php?action=delete&Id=<?php echo $values["ID"]; ?>" class="font-bold text-white rounded-full bg-red-500 border border-red-100 p-2 hover:underline">Remove</a>
                </td>
            </tr>
                 ...
                 <?php $total = $total + ($values["Quantité"] * $values["Prix"]); } ?>
					...
					<tr align="center" class="bg-red-300 border-b text-xl dark:bg-gray-800 dark:border-gray-700">
						...
						<td class="th"><a href="glaces.php?action=clearall"><button class="btn btn-red-500 text-white rounded-lg">clear all</button></a></td>
